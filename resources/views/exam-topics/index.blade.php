<x-admin-layout sweetalert="1">
    <div class="lg:container px-3 py-3">
        <div class="mb-2 flex flex-wrap justify-between items-center">
            <div class="">
                <h1 class="text-2xl font-sans font-semibold">Exam Topics</h1>
            </div>
            <form action="{{ route('exam-topics.store') }}" method="post" data-js="app-create-form">
                    ...
                    <tbody>
                        @foreach($items as $item)
                            <tr class="border border-b border-gray-100">
                                ...
                                <td class="px-2 py-2">{{ $item?->chapter?->name }}</td>
                                ...
                                <td class="px-2 py-2">
                                    <div class="inline-flex flex items-center flex-wrap gap-1 justify-end">
                                        <x-button href="{{ route('exam-topics.edit', $item->id) }}" size="sm">Edit</x-button>
                                        <form action="{{ route('exam-topics.destroy', $item->id) }}" method="post" data-js="app-delete-form">
                                            @method('DELETE')
