<x-admin-layout sweetalert="1">
    <div class="lg:container px-3 py-3">
        <div class="mb-2 flex flex-wrap justify-between items-center">
            <div class="">
                <h1 class="text-2xl font-sans font-semibold">{{ $exam->name }}</h1>
            </div>
            <form action="{{ route('exams.add-question', $exam) }}" method="post" data-js="app-create-form">
                <x-button data-js="app-form-btn" type="submit">Add Question</x-button>
            </form>
        </div>
                            ...
                            <tr class="border border-b border-gray-100">
                                ...
                                <td class="px-2 py-2">
                                    <div class="inline-flex flex items-center flex-wrap gap-1 justify-end">
                                        <x-button href="{{ route('exams.edit-question', ['exam'=> $exam->id, 'id'=> $item->id]) }}" size="sm">Edit</x-button>
                                        <form action="{{ route('exams.destroy-question', ['exam'=> $exam->id, 'id'=> $item->id]) }}" method="post" data-js="app-delete-form">
                                            @method('DELETE')
